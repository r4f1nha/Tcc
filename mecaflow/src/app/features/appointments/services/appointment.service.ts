...
export class AppointmentService {
  ...
  getAll(filters: AppointmentFilters): Observable<ReadonlyArray<Appointment>> {
    ...
    if (filters.agentId) {
      params = params.set('agentId', String(filters.agentId));
    }
  ...
  update(
    ...
    return this.http.put<Appointment>(`${this.API}/${id}`, payload);
  }
  ...
  confirm(id: string | number): Observable<Appointment> {
    return this.http.patch<Appointment>(`${this.API}/${id}/confirm`, {});
  }

  cancel(id: string | number): Observable<Appointment> {
  ...
  getAvailableSlots(date: string): Observable<ReadonlyArray<TimeSlot>> {
    ...
    return this.http.get<ReadonlyArray<TimeSlot>>(`${this.API}/slots`, {
      params,
    });
  }
  ...
  getBlockedSlots(date: string): Observable<ReadonlyArray<BlockedSlot>> {
    ...
    return this.http.get<ReadonlyArray<BlockedSlot>>(`${this.API}/blocked`, {
      params,
    });
  }
} 